...
SELECT row_number()over(PARTITION BY p.person_id ORDER BY p.person_id) AS observation_period_id,
       ...
       CASE
            WHEN fgi.APPROX_BIRTH_DATE IS NULL AND fgi.BL_YEAR IS NOT NULL AND fgi.BL_AGE IS NOT NULL THEN DATE_SUB(PARSE_DATE("%Y",CAST(fgi.BL_YEAR AS STRING)), INTERVAL CAST(fgi.BL_AGE AS INT64) YEAR)
            ELSE fgi.APPROX_BIRTH_DATE
        END AS observation_period_start_date,
       CASE
            WHEN fgi.APPROX_BIRTH_DATE IS NULL AND fgi.BL_YEAR IS NOT NULL AND fgi.BL_AGE IS NOT NULL THEN DATE_ADD( DATE_SUB(PARSE_DATE("%Y",CAST(fgi.BL_YEAR AS STRING)), INTERVAL CAST(fgi.BL_AGE AS INT64) YEAR) , INTERVAL CAST(fgi.FU_END_AGE AS INT64) YEAR)
