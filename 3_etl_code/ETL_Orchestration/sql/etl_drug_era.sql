...
SELECT ROW_NUMBER() OVER (ORDER BY ctefinal.person_id) AS drug_era_id,
       ...
       DATEDIFF(DAY, MIN(ctefinal.drug_exposure_start_date),ctefinal.era_end_date) as gap_days
FROM (
SELECT d.person_id,
...
FROM (
  SELECT ROW_NUMBER() OVER (ORDER BY de.person_id) AS row_num,
         ...
         COALESCE(de.drug_exposure_end_date, DATEADD(d, de.days_supply ,  de.drug_exposure_start_date), de.drug_exposure_start_date) AS drug_exposure_end_date,
...
INNER JOIN (
  SELECT person_id,
         ...
         DATEADD(d, -120, event_date) AS end_date
  FROM(
  SELECT rawdata.person_id,
        ...
        FROM (
            ...
            UNION ALL
            SELECT person_id, ingredient_concept_id, DATEADD(d, 120, drug_exposure_end_date), 1 AS event_type, NULL
            FROM (
              SELECT ROW_NUMBER() OVER (ORDER BY de.person_id) AS row_num,
                     de.person_id AS person_id,
                     de.drug_type_concept_id AS drug_type_concept_id,
