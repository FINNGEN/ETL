# DESCRIPTION:
# Creates a row in cdm-visit occurrence table for each event of FinnGen id in the spirometry.
# Person id is extracted from person table.
# visit_occurrence_id is added by an offset of 120000000000
#
# PARAMETERS:
#
...
WITH
# 1- Collect all visits from spirometry registry with necesary columns
# - Only collect unique columns based on FINNGENID, SOURCE and APPROX_DATE
visits_from_spirometry AS (
  SELECT
      FINNGENID,
      CASE
  ...
  FROM (
    ...
    FROM (
      SELECT FINNGENID,
             APPROX_DATE,
             SOURCE,
             ROW_NUMBER() OVER(PARTITION BY FINNGENID, SOURCE, CAST(APPROX_DATE AS STRING) ORDER BY APPROX_DATE) AS q1
      FROM @schema_table_spirometry
      WHERE APPROX_DATE IS NOT NULL
    )
    WHERE q1 = 1
  )
),
...
SELECT
# visit_occurrence_id
  ROW_NUMBER() OVER(ORDER BY vfrwssvtf.FINNGENID) + 120000000000 AS visit_occurrence_id,
#person_id,
