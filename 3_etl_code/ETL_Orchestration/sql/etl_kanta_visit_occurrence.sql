...
visits_from_kanta AS (
  SELECT
      ...
      CODING_SYSTEM_OID AS SERVICE_PROVIDER
  FROM (
    SELECT FINNGENID,
           APPROX_EVENT_DATETIME,
           CODING_SYSTEM_OID
...
visit_type_fg_codes_preprocessed AS (
  ...
  FROM visits_from_kanta
),
# 2-2 append visit type from fg_codes_info_v7 table with non-standard code
...
ON CAST(vfrwssvtf.SERVICE_PROVIDER AS STRING) = provider.provider_source_value
