/*
 For comparision, the ohdsi versio of the drug_era generation.
 This could not be run at Pasi's dev environment where postgreSQL is in Docker container from Broadsea:
 could not resize shared memory segment "/PostgreSQL.448252718" to 33554432 bytes: No space left on device
...
(-- Normalize DRUG_EXPOSURE_END_DATE to either the existing drug exposure end date, or add days supply, or add 1 day to the start date
    ...
    FROM @schema_cdm_output.drug_exposure d
        JOIN @schema_vocab.concept_ancestor ca ON ca.descendant_concept_id = d.drug_concept_id
        JOIN @schema_vocab.concept c ON ca.ancestor_concept_id = c.concept_id
...
INSERT INTO @schema_cdm_output.drug_era(drug_era_id,person_id, drug_concept_id, drug_era_start_date, drug_era_end_date, drug_exposure_count, gap_days)
SELECT * FROM #tmp_de;