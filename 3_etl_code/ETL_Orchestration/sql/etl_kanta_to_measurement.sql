...
# measurement_id is added by an offset of 115000000000
#
#
# PARAMETERS:
#
...
lab_tests_with_measurement_source_concept_id AS (
  ...
  ON CONCAT(ltk.TEST_NAME_ABBREVIATION,'[',ltk.MEASUREMENT_UNIT,']') = fgc.code
),
...
lab_tests_with_measurement_and_unit_source_concept_id AS (
  ...
  LEFT JOIN ( SELECT code,
                     omop_concept_id
              FROM FROM @schema_table_codes_info
...
SELECT
# measurement_id
  ROW_NUMBER() OVER(ORDER by ltwmussci.FINNGENID) + 115000000000 AS measurement_id,
# person_id
  p.person_id AS person_id,
# measurement_concept_id
  CASE
      WHEN ltwmussci.conceptId IS NOT NULL THEN ltwmussci.conceptId
      ELSE 0
  END AS measurement_concept_id,
# measurement_date
  EXTRACT(DATE FROM ltwmussci.TEST_DATE_TIME) AS measurement_date,
# measurement_datetime
  ltwmussci.TEST_DATE_TIME AS measurement_datetime,
# measurement_time
  CAST(EXTRACT(TIME FROM ltwmussci.TEST_DATE_TIME) AS STRING) AS measurement_time,
# measurement_type_concept_id
...
# value_as_number - Negative values will only be accepted if the concept_id is from OHDSI prescribed standard concepts
  CASE
       WHEN SAFE_CAST(ltwmussci.MEASUREMENT_VALUE AS FLOAT64) < 0 AND
            ltwmussci.conceptId IN (3003396,3004959,3012501,3003129,3002032,3007435,3051343,21494211,21494205,21492970,21494210,21494204,21492969,
                                    3052504,3049581,21494209,21494203,21492968,21494208,21494202,21494199,3021722,3051450,21494100,3051938,21494201,
                                    21494198,3049418,21494200,21494197,3052486,3049278,21494099,36203243,36203246,36204416,36203244,36203245,36204415,
                                    36203242,36204445,36204414,3033804,3049654,36204417,21494207,21492972,21494196,21494206,21492971,21494101,3002101,
                                    3049273,21494098,3006277,3028303,3025448) THEN SAFE_CAST(ltwmussci.MEASUREMENT_VALUE AS FLOAT64)
       ELSE SAFE_CAST(ltwmussci.MEASUREMENT_VALUE AS FLOAT64)
...
# range_low
  SAFE_CAST(TEST_REFERENCE_MIN_VALUE AS FLOAT64) AS range_low,
# range_high
  SAFE_CAST(TEST_REFERENCE_MAX_VALUE AS FLOAT64) AS range_high,
# provider_id
...
# measurement_source_value
 ltwmussci.TEST_NAME_ABBREVIATION_SOURCE AS measurement_source_value,
# measurement_source_concept_id
  CASE
...
ON vo.person_id = p.person_id AND
   ...
   EXTRACT(DATE FROM ltwmussci.TEST_DATE_TIME) = vo.visit_start_date
