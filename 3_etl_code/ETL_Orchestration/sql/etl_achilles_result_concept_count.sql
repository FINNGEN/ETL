/* 
This populates the table that is required for Atlas to show record counts when searching concepts
This data is based on Achilles analysis results, so Achilles must be run before this.
...
WITH concepts AS (
  ...
  from (
    ...
    from #tmp_counts c
    join @schema_vocab.concept_ancestor ca on cast(ca.descendant_concept_id as varchar(50)) = c.concept_id
  ) c
  left join @schema_vocab.concept_ancestor ca on c.concept_id = cast(ca.ancestor_concept_id as varchar(50))
)
SELECT ancestor_id,
  descendant_id
INTO #tmp_concepts
