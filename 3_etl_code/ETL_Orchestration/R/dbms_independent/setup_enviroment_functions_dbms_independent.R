...
create_tables_and_load_vocabulary <- function(config) {
  ...
  sql <- SqlRender::readSql("sql/dbms_independent/setup_create_vocab_OMOPCDM_5.4_ddl_dbms_independent.sql")
  ...
  # load vocabularies
  sql <- SqlRender::readSql("sql/dbms_independent/setup_load_vocabularies_from_csv_postgresql.sql")
...
create_tables_etl_input_and_output_cdm <- function(config) {
  ...
  sql <- SqlRender::readSql("sql/dbms_independent/setup_create_etl_input_tables_ddl_dbms_independent.sql")
  ...
  # Create  cdm  vocab tables -------------------------------------------------
  sql <- SqlRender::readSql("sql/dbms_independent/setup_create_cdm_OMOPCDM_5.4_ddl_dbms_independent.sql")
...
transform_and_copy_source_tables_to_etl_input <- function(config) {
  ...
  sql <- SqlRender::readSql("sql/dbms_independent/setup_transform_service_sector_minimum_finngenid_info_to_etl_input_tables_dbms_independent.sql")
