...
# Declare Test - 1201 - Default values
declareTest(1201, "etl_condition_occurrence adds two rows for default values for source birth_mother")
...
declareTest(1202, "etl_condition_occurrence adds one row for code with standard mapping for source birth_mother")
...
# Declare Test - 1203 - Mother with two diagnosis codes that are non-null and both fall in condition domain
declareTest(1203, "etl_condition_occurrence adds two rows for two diagnosis codes with standard mapping in condition domain for source birth_mother")
...
declareTest(1204, "etl_condition_occurrence adds only one row for the diagnosis code with standard mapping in condition but not for diagnosis code in procedure domain for source birth_mother")
...
# Declare Test - 1205 - Mother with diagnosis code that is non-null but has standard maps to both condition and procedure domain
declareTest(1205, "etl_condition_occurrence adds only one row for a diagnosis code with standard mapping to 2 codes one condition and one procedure for source birth_mother")
...
declareTest(1206, "etl_condition_occurrence adds a row for a diagnosis code with no standard mapping for source birth_mother")
...
# Declare Test - 1207 - Mother with diagnosis code that is non-null but has no non-standard map
declareTest(1207, "etl_condition_occurrence adds a row for a diagnosis code with no non-standard mapping for source birth_mother")
