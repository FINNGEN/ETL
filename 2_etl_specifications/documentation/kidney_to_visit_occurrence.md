...
| --- | --- | --- | --- |
| visit_occurrence_id |  | Incremental integer.<br> Unique value per `finngenid` + 114000000000 (offset). | Generated |
...
| visit_start_date | approx_event_day<br>year | `visit_start_date` is directly taken from `approx_event_day` when not NULL <br> when NULL then end date of the `year` will be used. Ex: If `year` is 2019 then 2019-12-31 will be the `visit_start_date` | Calculated |
...
| visit_source_value |  | String build as  "SOURCE=KIDNEY;INDEX=" | Calculated |
| visit_source_concept_id |  | `omop_concept_id` from fg_codes_info where `source` equals "KIDNEY" | Calculated using the fg_codes_info table. |
