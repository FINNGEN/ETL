...
| visit_start_date | medication_approx_event_day | Copied from `medication_approx_event_day`  | Copied |
...
| visit_source_concept_id |  | `omop_source_concept_id` from fg_codes_info where `source` IN ("DELIVERY", "KELA", "PRESCRIPTION_DELIVERY", "DELIVERY_KELA", "PRESCRIPTION_DELIVERY_KELA") and `vocabulary_id` equals "FGvisitType" <br> ELSE 0  | Calculated using the fg_codes_info table |
