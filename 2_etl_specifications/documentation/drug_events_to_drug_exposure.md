...
| --- | --- | --- | --- |
| drug_exposure_id |  |   Incremental integer. <br>  Unique value per each row in drug_exposure <br> `merged_source` is used to select rows that are not prescription only by `merged_source` != 'PRESCRIPTION' |  Generated |
...
| drug_exposure_start_date | medication_approx_event_day | Copied directly from `medication_approx_event_day` | Copied |
| drug_exposure_start_datetime | | Calculated from  `drug_exposure_start_date` with time 00:00:0000 | Calculated |
| drug_exposure_end_date |  medication_vnr<br>medication_quantity<br>finngen_vnr | `medication_vnr` is looked up in finngen_vnr to get `DDDPerPack` which is considered as days_supply. `drug_exposure_end_date` = `durg_exposure_start_date` + `DDDPerPack` * `medication_quantity`. | Calculated |
...
| drug_source_value | medication_vnr |  Copied adding 6 leading zeroes LPAD(drug_events.medication_vnr,6,'0') | Copied   |
| drug_source_concept_id | medication_vnr | `omop_concept_id` from fg_codes_info where `source` equals "PURCH" `medication_vnr` equals `FG_CODE3`  |  Calculated   |
